...
class MongoDBManager {
  ...
  private async createIndexes() {
    ...
    await this.db.collection("chat_sessions").createIndexes([
    ...
    // Checkpoints indexes
    await this.db.collection("checkpoints").createIndexes([
    ...
    await this.db.collection("agent_interactions").createIndexes([
      { key: { threadId: 1, timestamp: -1 } },
...
export class LongTermMemoryStore {
  ...
  async put(
    ...
    await this.db.collection("long_term_memory").updateOne(
      { _id: memory._id },
