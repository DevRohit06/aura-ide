...
async function main() {
  ...
  try {
    ...
    const result1 = await app.invoke({
      messages: [new HumanMessage("Create a Python function to calculate fibonacci numbers and test it with n=10")],
    });
    ...
    const result2 = await app.invoke({
      messages: [new HumanMessage("Create a Python script to analyze a list of numbers [1,5,3,8,2,9,4] and show mean, median, and standard deviation")],
    });
...
async function enhancedExample() {
  ...
  try {
    ...
    const result = await app.invoke({
      messages: [new HumanMessage("Create a TypeScript class for managing a todo list with add, remove, and list methods")],
    });
...
if (require.main === module) {
  ...
  (async () => {
    ...
    try {
      const result = await app.invoke({
        messages: [new HumanMessage("Write a Python function to sort a list of dictionaries by a specific key")],
      });
